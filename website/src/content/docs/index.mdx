---
title: Coldspa
description: Give your CFML a spa day. The Islands Architecture for ColdFusion -- mount Vue or React components into CFML pages with server-side rendering and progressive hydration.
template: splash
hero:
  tagline: Give your CFML a spa day. The Islands Architecture for ColdFusion.
  image:
    file: ../../../public/coldspa.svg
  actions:
    - text: Get started
      link: /guides/getting-started/
      icon: right-arrow
    - text: View on GitHub
      link: https://github.com/crutchcorn/coldspa
      icon: external
      variant: minimal
---

import { Card, CardGrid } from '@astrojs/starlight/components';

Mount Vue or React components into CFML pages with server-side rendering and progressive hydration. Coldspa brings the islands architecture popularized by Astro to ColdFusion, so you can keep rendering pages with CFML while sprinkling in interactive widgets that hydrate on demand.

## Why Coldspa?

<CardGrid stagger>
	<Card title="CFML-first" icon="seti:coldfusion">
		Use `<cf_Island>` and `<cf_Slot>` like any other custom tags. Pass CFML structs as props and CFML markup as slots.
	</Card>
	<Card title="Real SSR" icon="rocket">
		A Node sidecar pre-renders Vue and React to HTML so the page is meaningful before JS loads -- no flash of empty containers.
	</Card>
	<Card title="Progressive hydration" icon="setting">
		Choose `load`, `idle`, `visible`, or `client` per island. Below-the-fold widgets stay dormant until the user scrolls.
	</Card>
	<Card title="Vite-powered" icon="seti:javascript">
		HMR in dev, hashed bundles + manifest in prod. `coldspa/vite` wires up component globs, client entries, and the SSR build alongside your normal framework plugins.
	</Card>
	<Card title="Vue & React" icon="seti:vue">
		Both frameworks supported out of the box, and you can mix them on the same page.
	</Card>
	<Card title="Slots, the CFML way" icon="document">
		Default and named slots come from CFML markup -- including `<cfoutput>` queries, `<cfinclude>`, and conditionals.
	</Card>
</CardGrid>
